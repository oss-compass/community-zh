# 2024-12-24开源指南针会议纪要
## 时间
北京时间2024年12月24日下午20:00-21:00
## 与会人
王晔晖、张益翔、赵海玲、胡骁杰、潘雷明、徐榕蔓、齐国强、赖兴友、李松楠、李春敏
## 主题——Compass社区工作进度讨论
### 一、讨论开源之夏与Compass合作事宜（张益翔）
#### 1.内容块聚合与语义信息提取
讨论了内容块和聚合网络的构建，以及视频和图像信息的提取。视频信息包括链接和动图两种形式，对于链接形式的处理较为困难，目前采用普通链接的方式处理。语义信息方面，通过五个特征进行评分，并尝试使用模型方式进行表征。此外，还讨论了文本丰富度的增加，以及多模态方式对内容丰富度的影响。最后，提到了可读性和时效性的重要性。
#### 2.文本可读性与时效性讨论
讨论了文本的可读性评估，包括段落数量、段落符号、图片与图片段落、标题长度正文特征等方面。同时，提到了时效性评估，通过比较发布版本时间和更新时间来判断。目前，这部分工作尚未完成，计划在春节前完成。之后，王晔晖提出会与中科院合作一个长期项目，需要长时间投入。
#### 3.论文布局研究讨论
关于论文布局，齐国强提到了可以参考其他研究者的模型，并在论文中提出自己的创新点。此外，还讨论了文档更新的问题，包括版本发布和文档内容匹配等。关于可读性评估的延续和创新，齐国强提出需要在已有基础上进行特征提取和对比分析。最后大家讨论了论文的研究问题、研究对象、技术创新和软件工程等方面，重点探讨了API治理、资产管理、知识库建设等技术问题，以及如何确保论文的延续性和方法创新。
### 二、关于OSS-Compass的洞察&思考（徐榕蔓）
徐榕蔓提出在操作的时候遇到了一些问题：REST API改成了HTTP的形式进行请求，但获取信息的时候，显示报错。对于该问题，王晔晖表示后续出现这些问题可以去找赖兴友提供帮助。
### 三、讨论开放原子与Compass的合作事宜（赵海玲、胡骁杰）
关于数据方面的进展，王晔晖表示目前工作正在有序进行，全球洞察基本完成，各国家进出口开源工程进出口图已制作完成，接下来将进行2024年的研究，包括国内技术水域分析。预计月底完成数据处理和设计工作，1月左右出一个初稿。另外关于专委会的组建，赵海玲表示已经与理事长沟通，预计明年年初会开展起来。后续如有需要，可以随时提供支持。                                                                      
### 四、介绍Openlinksaas产品及其合作可能性（潘雷明）
潘雷明作为Openlinksaas 创始人，在会上介绍了产品，主要功能包括整合研发所需工具、统计分析功能、健康度评价系统等。此外，还提到了一些工具如远程终端访问数据库、安全发布等，以及项目管理、沟通协作、团队管理等功能。关于客户端和服务端的信息协作问题，以及高校和企业对解决方案的需求，王晔晖提出可以尝试使用Compass去获取想要的数据。最后共同讨论了关于工具的服务场景、常规功能、SaaS接口和扩展功能，如健康度指标、代码合规性和缺陷检测等。
### 五、遗留问题
●下周制定一个阶段性成果的demo
（责任人：张益翔；完成时间：2024年12月31日）

●在1月第二周完成数据处理和设计工作，并准备初稿以便进行数据洞察布置
（责任人：王晔晖；完成时间：1月中旬）

●3.在农历春节前制定并发布Compass社区年报
（责任人：李春敏；完成时间1月中旬）
# 2024-12-17开源指南针会议纪要
## 时间
北京时间2024年12月17日下午20:00-20:30
## 与会人
王晔晖、张盛翔、张益翔、齐国强、李升保、李春敏
## 主题——Compass社区工作进度讨论
### 一、讨论开源之夏与Compass合作事宜（张益翔）
#### 1.项目特征提取与布局优化
讨论了以下几个方面：a.关于特征提取和位置信息的相关问题，包括图片的相对位置和渲染效果；b.关于视频的处理方式，主要是以网站形式存储，放到readme文档里显示；c.关于项目介绍的结构，如项目架构、联系方式、bug处理情况等，以及如何对不同段落进行分类和颜色标记，以便更好地分析项目的特征。
#### 2.项目简介与使用指南的深度解析
主要讨论了语义特征网络中的聚类效果，以及如何提取图片内部的文本信息。王晔晖和齐国强提到了项目简介、场景说明、使用说明、开发者指南和贡献指南等部分，并对相关内容进行了解释说明。
#### 3.图片处理与信息提取的探讨
讨论了图片处理的问题。对于logo形式的图片，张益翔提出可以将其视为一种额外废图，直接过滤掉。齐国强提出，不建议对图片通过模型进行OCR提取文字，而是只需要应该关注图片是否能够反映信息类型，以及它与使用说明的相关性。最后，关于视觉优化的问题，张盛翔提出可以通过计算单位空间内的图片数量来优化。
### 二、遗留问题
无

# 2024-12-03开源指南针会议纪要
## 时间
北京时间2024年12月3日下午20:00-21:00
## 与会人
王晔晖、张盛翔、张益翔、徐榕蔓、赵海玲、胡骁杰、郭昕竺、齐国强、李升保、赖兴友、李松楠、李春敏
## 主题——Compass社区工作进度讨论
### 一、讨论开源之夏与Compass合作事宜（张益翔）
#### 1.关于layout以及文本丰富度等问题讨论
关于布局问题，张益翔采用了markdown格式，但存在图片高度信息和位置信息不统一的问题；关于聚类问题，通过二维散点图展示了不同类别之间的过渡情况，讨论了迭代次数和数据均不均衡对聚类结果的影响；关于中文和英文分词问题，张益翔将中文文本摘出来，但未对英文进行处理。张盛翔提出了两个思路：一是按照布局部分继续深入，对内容进行定义，赋予权重，对段落长度进行约束；二是对图像的宽高比进行计算，以确定其在屏幕上的显示效果。这两个思路可以帮助解决布局问题，提高论文的可读性和实用性。另外大家讨论了论文中关于四类在不同象限分布的验证，以及如何通过特征提取来解决位置信息不统一的问题。
#### 2.优化文章结构、图片处理方式
关于文章结构：讨论了如何对视觉上的效果进行量化，通过代码块和文本处理方式来贴近文本，同时考虑代码块的行数对视觉高度的影响；另外探讨了如何剥离平台对CSS或渲染样式的依赖，以及如何识别文章的结构和内容。

关于图片处理方式：张盛翔提到了可以通过机器学习的方式对项目图片进行标识，以便在视觉上实现差异化；另外关于如何将图片分成包含内容信息和结构信息两大类，以及如何识别和提取图片中的文本信息。张盛翔提到了从视觉上分析图片的宽高比，判断其形态，进而识别和提取相关信息。
### 二、关于OSS-Compass的洞察&思考（徐榕蔓）
#### 1.API文档编写
讨论了Compass项目的进展，包括通过rest形式书写GQL，使用swagger editor调试并提出了无法访问GQL API的问题，后续可以求助赖兴友帮忙解决。
### 三、讨论开放原子与Compass的合作事宜
#### 1.Github全球洞察数据分析
李升保分享了2024年数据的收集和开发者画像算法的验证情况，包括基于Github innovation graph等一些统计数据，最后王晔晖提出后续会和Gitee进行合作获取更多全面的数据。
#### 2.数据库操作系统项目分析进展
齐国强分享了本月开放原子峰会涉及到的榜单数据，包括数据库操作系统等进展。分享提出目前有80多万个项目，其中操作系统相关的项目有4000多个，数据库相关项目有8000多个。通过模型分析，识别出25万多个项目。接下来会结合项目画像，将中国部分和剩余部分分别拎出来，用相同的方法和模型进行分类，王晔晖建议在排名时考虑一些生态相关的优秀项目。

针对此前洞察方式，王晔晖提出希望后续可以更加注重数据库的生态，齐国强与胡骁杰展开讨论，认为可以围绕数据库的几个方向分类去进行洞察，这样也可以对原有报告进行多维度充实，并提出希望看到行业类别趋势和分布，以及技术领域在哪些行业或产业能够有落地应用和商业化。

另外，关于在开放原子成立类似于LF Research的部门事宜，赵海玲提出后续将会邀请理事长确认相关事宜，并计划在12月10号左右邀请大家来线下进行汇报讨论。
### 四、遗留问题
●后续与Gitee合作，获取更多更全面的数据以补充并验证相关观点（责任人：王晔晖；完成时间：2024年12月10日）

●确认并安排与开放原子理事长的会议时间，以便就成立类似于LF Research的部门进行详细汇报（责任人：赵海玲；完成时间：2024年12月10日）
